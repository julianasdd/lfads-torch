...
data_path = '/Volumes/gaia.tavoni/Active/naranjorincon/temp_dynamics_VAE-RNN/lfads-torch/data_extracted/monk_p/COT_SpikesCombined/';  %Change to data path of original data
...
SpikeTimes = SpikeTimes(channelsIncluded,:); % M1 units that are in the channels defined above
%% extract max/min spiketime
for jj = 1:length(SpikeTimes)
    for ii = 1:length(TrialInfo.trial_start_time) % time when given trial started
        if isempty(SpikeTimes{jj,1}{1,ii}) == 0 %not empty
...
max_SpikeTime = max(max_spiketime); %largest trial end time for each unit across all their 1040 trials
min_SpikeTime = min(min_spiketime);
%% Config
bin=1;  %10 ms, 100 Hz
timeMs = min_SpikeTime*1000:bin:max_SpikeTime*1000; %in Ms, % so, finds max time and makes all trials that size even tho that can create gaps of NaNs (or 0s) for short trials 
...
cat_spikes = reshape(spikes, size(spikes,1),[]); % combines trials and time dims
...
spikes = permute(trial_histcounts,[3 2 1]);%sort as neuron x time x trial for LFADS
rng('default');
t0_mat = randi([1 200/bin+1],1,size(spikes,3)); %1xtrial array of rand trial nums, min1 max11
